...
.cs-brand {{ display:flex; align-items:center; padding-top:2px; margin-bottom:22px; }}
.cs-logo {{ display:inline-flex; align-items:flex-start; gap:0; }}
.cs-logo-word {{ font-family:{DISPLAY}; font-weight:800; font-size:27px; letter-spacing:-.035em;
  color:#111318 !important; line-height:1; }}
.cs-logo-arrow {{ margin:-3px 0 0 1px; display:block; }}
...
# "kairos" wordmark — black text with a red rising arrow (replicated from the brand logo).
KAIROS_LOGO = (
    "<div class='cs-logo'>"
    "<span class='cs-logo-word'>kairos</span>"
    "<svg class='cs-logo-arrow' width='19' height='19' viewBox='0 0 28 28' fill='none' "
    "xmlns='http://www.w3.org/2000/svg'>"
    "<path d='M3.5 21.5C11 21.5 18.5 17.5 22.5 7' stroke='#E2231A' stroke-width='3.6' "
    "stroke-linecap='round'/>"
    "<path d='M15.5 5.5L23.5 5.2L23 13.2' stroke='#E2231A' stroke-width='3.6' "
    "stroke-linecap='round' stroke-linejoin='round'/></svg>"
    "</div>")


def step_rail(order: list[str], labels: dict[str, str], current: int):
    rows = [f'<div class="cs-brand">{KAIROS_LOGO}</div>']
